...
int remove_watch(int fd, const char *path) {
	struct watch *w;

	for (w = watches; w; w = w->next) {
		if (strcmp(path, w->path) == 0) return (inotify_rm_watch(fd, w->wd));
	}

	return 1;
}

unsigned int next_highest(unsigned int num) {
	num |= num >> 1;
	num |= num >> 2;
	num |= num >> 4;
	num |= num >> 8;
...
int inotify_create_watches(int fd) {
	FILE *max_watches;
	...
	for (i = 1; i <= rows; i++) {
		...
		if (add_watch(fd, result[i]) == -1) {
			// This happens when a directory is completely removed, then the binary is
			// started.
			// TODO: In this case, the object should be removed from table, or table
	...
	if (max_watches) {
		if (fscanf(max_watches, "%10u", &watch_limit) < 1) watch_limit = 8192;
		fclose(max_watches);
		if ((watch_limit < DESIRED_WATCH_LIMIT) ||
				(watch_limit < (num_watches * 4 / 3))) {
			...
			if (max_watches) {
				...
				DPRINTF(E_WARN, L_INOTIFY,
								"WARNING: Inotify max_user_watches [%u] is low or close to the "
								"number of used watches [%u] "
								"and I do not have permission to increase this limit.  Please "
								"do so manually by "
		...
		DPRINTF(E_WARN, L_INOTIFY,
						...
						"Hopefully it is enough to cover %u current directories plus any "
						"new ones added.\n",
						num_watches);
	}
...
int inotify_remove_watches(int fd) {
	struct watch *w = watches;
	struct watch *last_w;
	int rm_watches = 0;
...
int add_dir_watch(int fd, char *path, char *filename) {
	...
	wd = add_watch(fd, dir);
	if (wd == -1) {
		DPRINTF(E_ERROR, L_INOTIFY, "add_watch() [%s] %s\n", strerror(errno), dir);
	} else {
...
int inotify_insert_file(char *name, const char *path) {
	...
	*/
	...
	switch (types) {
		case ALL_MEDIA:
			if (!is_image(path) && !is_audio(path) && !is_video(path) &&
					!is_playlist(path))
				return -1;
			break;
		case TYPE_AUDIO:
			if (!is_audio(path) && !is_playlist(path)) return -1;
			break;
		case TYPE_AUDIO | TYPE_VIDEO:
			if (!is_audio(path) && !is_video(path) && !is_playlist(path)) break;
		case TYPE_AUDIO | TYPE_IMAGES:
			if (!is_image(path) && !is_audio(path) && !is_playlist(path)) return -1;
			break;
		case TYPE_VIDEO:
			if (!is_video(path)) return -1;
			break;
		case TYPE_VIDEO | TYPE_IMAGES:
			if (!is_image(path) && !is_video(path)) return -1;
			break;
		case TYPE_IMAGES:
			if (!is_image(path)) return -1;
			break;
		default:
			return -1;
			break;
	...
	ts = sql_get_int_field(db, "SELECT TIMESTAMP from DETAILS where PATH = '%q'",
												path);
	if (!ts && is_playlist(path) &&
			(sql_get_int_field(db, "SELECT ID from PLAYLISTS where PATH = '%q'",
												path) > 0)) {
	...
	len = strlen(path) + 1;
	if (!(path_buf = malloc(len)) || !(last_dir = malloc(len)) ||
	...
	while (depth) {
		...
		do {
			...
			id = sql_get_text_field(db,
															"SELECT OBJECT_ID from OBJECTS o left join "
			...
			if (id) {
				...
				insert_directory(base_copy, last_dir, BROWSEDIR_ID, id + 2,
	...
	if (!depth) {
		...
		if ((is_audio(path) || is_playlist(path)) && next_pl_fill != 1) {
			next_pl_fill =
					time(NULL) + 120;  // Schedule a playlist scan for 2 minutes from now.
			// DEBUG DPRINTF(E_WARN, L_INOTIFY,  "Playlist scan scheduled for %s",
			// ctime(&next_pl_fill));
		}
	}
	return depth;
...
int inotify_insert_directory(int fd, char *name, const char *path) {
	DIR *ds;
	struct dirent *e;
	...
	if (sql_get_int_field(db, "SELECT ID from DETAILS where PATH = '%q'", path) >
			0) {
	...
	id = sql_get_text_field(db,
													"SELECT OBJECT_ID from OBJECTS o left join DETAILS d "
													"on (d.ID = o.DETAIL_ID)"
													" where d.PATH = '%q' and REF_ID is NULL",
													dirname(parent_buf));
	if (!id) id = sqlite3_mprintf("%s", BROWSEDIR_ID);
...
int inotify_remove_file(const char *path) {
	char sql[128];
	...
	/* Invalidate the scanner cache so we don't insert files into non-existent
	* containers */
	valid_cache = 0;
	playlist = is_playlist(path);
	id = sql_get_text_field(db, "SELECT ID from %s where PATH = '%q'",
													playlist ? "PLAYLISTS" : "DETAILS", path);
	if (!id) return 1;
	...
	if (playlist) {
		...
		sql_exec(db,
						"DELETE from OBJECTS where OBJECT_ID = '%s$%llX' or PARENT_ID = "
						"'%s$%llX'",
		...
		if ((sql_get_table(db, sql, &result, &rows, NULL) == SQLITE_OK)) {
			...
			for (i = 1; i <= rows; i++) {
				...
				children = sql_get_int_field(
						db, "SELECT count(*) from OBJECTS where PARENT_ID = '%s'",
						result[i]);
				...
				if (children < 2) {
					...
					if (sql_get_int_field(
									db, "SELECT count(*) from OBJECTS where PARENT_ID = '%s'",
									result[i]) == 0) {
						sql_exec(db, "DELETE from OBJECTS where OBJECT_ID = '%s'",
										result[i]);
					}
				}
			}
...
int inotify_remove_directory(int fd, const char *path) {
	char *sql;
	char **result;
	int64_t detailID = 0;
	int rows, i, ret = 1;
	...
	* containers */
	...
	sql = sqlite3_mprintf(
			"SELECT ID from DETAILS where (PATH > '%q/' and PATH <= '%q/%c')"
			" or PATH = '%q'",
			path, path, 0xFF, path);
	if ((sql_get_table(db, sql, &result, &rows, NULL) == SQLITE_OK)) {
		if (rows) {
	...
	sql_exec(db, "DELETE from ALBUM_ART where (PATH > '%q/' and PATH <= '%q/%c')",
					path, path, 0xFF);
			...
			if (event->len) {
				...
				if (event->mask & IN_ISDIR &&
					...
					DPRINTF(E_DEBUG, L_INOTIFY, "The directory %s was %s.\n", path_buf,
									(event->mask & IN_MOVED_TO ? "moved here" : "created"));
					...
					if (S_ISLNK(st.st_mode)) {
						...
						if ((event->mask & IN_MOVED_TO) ||
								(sql_get_int_field(
										db, "SELECT TIMESTAMP from DETAILS where PATH = '%q'",
										path_buf) != st.st_mtime)) {
					...
					DPRINTF(E_DEBUG, L_INOTIFY, "The %s %s was %s.\n",
									(event->mask & IN_ISDIR ? "directory" : "file"), path_buf,
									(event->mask & IN_MOVED_FROM ? "moved away" : "deleted"));
					if (event->mask & IN_ISDIR)
